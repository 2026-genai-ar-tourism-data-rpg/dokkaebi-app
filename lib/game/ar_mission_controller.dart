...
// 여기 forward·right 숫자 자체(간격·좌우 배치 패턴)는 세션 시작 시점 카메라 기준이든
// 나침반 기준이든 그대로다 — "정면 12m에서 시작해 저쪽으로 이어지는" 배치 패턴만 정할 뿐,
// 그걸 실제 어느 축(카메라 vs 나침반)에 얹을지는 네이티브가 kind로 나눠 정한다(v8,
// DokkaebiArView.swift). VPS(세계좌표 앵커)는 여전히 없지만, 나침반(진북)은 VPS 없이도
// 기기 자체 센서로 되는 별개 기능이라 hunt·restore·find(coin·part·hidden)엔 그걸 쓴다 —
// "AR 켤 때 우연히 보던 방향"이 아니라 진짜 동서남북에 고정해 실행마다 같은 방향이 되게.
// photo(pattern)는 그대로 카메라 기준(실제 이미지 인식이 대신 위치를 잡아 준다).
